...
func (db *DB) SaveLink(link *domain.Link) error {
...
// GetLink is exported.
func (db *DB) GetLink(hash string) (*domain.Link, error) {
...
func (db *DB) UpdateLink(link *domain.Link) error {
	query := `
		UPDATE links
		SET source = ?, from_url = ?
		WHERE hash = ?
...
func (db *DB) SaveResource(link *domain.Resource) error {
...
// SaveLog is exported.
func (db *DB) SaveLog(log *domain.SysLog) error {
...
func (db *DB) LoadSources() ([]domain.Source, error) {
...
func (db *DB) LoadRules() ([]domain.Rule, error) {
